...
import org.geometerplus.zlibrary.ui.android.aplicatii.romanesti_molitfelnic.error.ErrorKeys;
import org.geometerplus.zlibrary.ui.android.aplicatii.romanesti_molitfelnic.library.ZLAndroidLibrary;
import org.geometerplus.zlibrary.ui.android.aplicatii.romanesti_molitfelnic.view.AndroidFontUtil;
import org.geometerplus.zlibrary.ui.android.aplicatii.romanesti_molitfelnic.view.MainView;
...
public final class FBReaderMolitfelnic extends FBReaderMainActivity implements ZLApplicationWindow {
	public static final int RESULT_DO_NOTHING = RESULT_FIRST_USER;
	public static final int RESULT_REPAINT = RESULT_FIRST_USER + 1;

	public static Intent defaultIntent(Context context) {
		return new Intent(context, FBReaderMolitfelnic.class)
				...
				synchronized (myPluginActions) {
					...
					for (PluginApi.ActionInfo info : myPluginActions) {
						myFBReaderApp.addAction(
							...
							new RunPluginAction(FBReaderMolitfelnic.this, myFBReaderApp, info.getId())
						);
					}
				}
			}
...
//						new TipRunner().start(); //aplicatii.romanesti disable tips runner (even if disabled in manifest, it's still running
						DictionaryUtil.init(FBReaderMolitfelnic.this, null);
						final Intent intent = getIntent();
						if (intent != null && FBReaderIntents.Action.PLUGIN.equals(intent.getAction())) {
							new RunPluginAction(FBReaderMolitfelnic.this, myFBReaderApp, intent.getData()).run();
						}
					}
				});
			}
...
//					if (myFBReaderApp.TestIfCopyIsRequired(FBReaderMolitfelnic.this)) { //se verfica si in functie oricum. //aplicatii.romanesti
						myFBReaderApp.copyBooksToSDCard(FBReaderMolitfelnic.this); //aplicatii.romanesti:  passing context
//					}
...
//		myFBReaderApp.addAction(ActionCode.SHOW_NETWORK_LIBRARY, new ShowNetworkLibraryAction(this, myFBReaderApp));//decluttering  by aplicatii.romanesti
			...
			final Runnable runnable = new Runnable() {
				public void run() {
					...
					if (myFBReaderApp.getTextView().search(pattern, true, false, false, false) != 0) {
						...
						runOnUiThread(new Runnable() {
							public void run() {
								UIMessageUtil.showErrorMessage(FBReaderMolitfelnic.this, "textNotFound");
		...
		Config.Instance().runOnConnect(new Runnable() {
			public void run() {
				...
				if (showStatusBar != myShowStatusBarFlag) {
					finish();
					startActivity(new Intent(FBReaderMolitfelnic.this, FBReaderMolitfelnic.class));
				}
				zlibrary.ShowStatusBarOption.saveSpecialValue();
				myFBReaderApp.ViewOptions.ColorProfileName.saveSpecialValue();
				SetScreenOrientationAction.setOrientation(FBReaderMolitfelnic.this, zlibrary.getOrientationOption().getValue());
			}
		});
	...
	private class TipRunner extends Thread {
		...
		public void run() {
			final TipsManager manager = new TipsManager(Paths.systemInfo(FBReaderMolitfelnic.this));
			switch (manager.requiredAction()) {
				case Initialize:
					startActivity(new Intent(
						TipsActivity.INITIALIZE_ACTION, null, FBReaderMolitfelnic.this, TipsActivity.class
					));
					break;
				case Show:
					startActivity(new Intent(
						TipsActivity.SHOW_TIP_ACTION, null, FBReaderMolitfelnic.this, TipsActivity.class
					));
					break;
				case Download:
	...
	protected void onResume() {
		...
		Config.Instance().runOnConnect(new Runnable() {
			public void run() {
				SyncOperations.enableSync(FBReaderMolitfelnic.this, myFBReaderApp.SyncOptions);
				...
				getCollection().bindToService(FBReaderMolitfelnic.this, new Runnable() {
					public void run() {
	...
	public boolean onSearchRequested() {
		...
		if (DeviceType.Instance().hasStandardSearchDialog()) {
			...
			SearchDialogUtil.showDialog(
				this, FBReaderMolitfelnic.class, myFBReaderApp.MiscOptions.TextSearchPattern.getValue(), new DialogInterface.OnCancelListener() {
					@Override
				...
				if (myWakeLockToCreate) {
					...
					myWakeLock =
						((PowerManager)getSystemService(POWER_SERVICE))
							.newWakeLock(PowerManager.SCREEN_BRIGHT_WAKE_LOCK, "FBReaderMolitfelnic");
					myWakeLock.acquire();
				}
			}
		}
